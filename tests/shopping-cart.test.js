...
function loadCart(initialValue = null) {
  const storage = {};
  if (initialValue !== null) storage['mariachi-fiesta-cart-v1'] = initialValue;
  const window = {
    localStorage: {
...
test('adds, merges, updates, removes, and persists cart items', () => {
  ...
  assert.equal(JSON.parse(storage['mariachi-fiesta-cart-v1'])[0].quantity, 4);
...
test('reconciles saved items with authoritative catalog data', () => {
  ...
  assert.deepEqual(snapshot(cart), [{ ...taco, name: 'Taco updated', price: 275 }]);
